...
- (void)startLoadingAnimate {
    NSString *bundlePath = [[[NSBundle bundleForClass:self.class] resourcePath] stringByAppendingPathComponent:@"LibUI.bundle"];
    NSBundle *bundle = [NSBundle bundleWithPath:bundlePath];
    NSURL *bundleURL = [bundle URLForResource:@"XCButtonLoading" withExtension:@"gif"];
    ...
    CFRelease(source);
    [self.loadingView setAnimationImages:images];
    [self.loadingView setAnimationDuration:duration];
...
- (void)layoutSubviews {
    ...
    self.loadingView.frame = CGRectMake((CGRectGetWidth(self.frame) - 24) / 2, (CGRectGetHeight(self.frame) - 24) / 2, 24, 24);
}
...
- (UIImageView *)loadingView {
    if (!_loadingView) {
        _loadingView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"xc_button_loading"]];
