...
XCAlertRectCorner const XCAlertRectCornerZero = {0, 0, 0, 0};

BOOL XCAlertRectCornerEqualZero(XCAlertRectCorner corner) {
...
XCAlertRectCorner XCAlertRectCornerMake(CGFloat topLeft, CGFloat topRight, CGFloat bottomLeft, CGFloat bottomRight) {
    XCAlertRectCorner corner = {topLeft, topRight, bottomLeft, bottomRight};
    return corner;
}

XCAlertRectCorner XCAlertRectCornerMakeAll(CGFloat all) {
    return XCAlertRectCornerMake(all, all, all, all);
}

XCAlertRectCorner XCAlertRectCornerMakeTopClips(CGFloat top) {
    return XCAlertRectCornerMake(top, top, 0, 0);
}

XCAlertRectCorner XCAlertRectCornerMakeBottomClips(CGFloat bottom) {
    return XCAlertRectCornerMake(0, 0, bottom, bottom);
}
...
@property (nonatomic, assign) XCAlertRectCorner rectCorner;
@end
...
- (void)layoutSubviews {
    ...
    CGRect bounds = self.bgMaskView.bounds;
    if (CGRectEqualToRect(bounds, CGRectZero)) {
        return;
    }
    UIBezierPath* rounded = [UIBezierPath bezierPath];
    [rounded moveToPoint:CGPointMake(bounds.size.width / 2, 0)];
    [rounded addLineToPoint:CGPointMake(self.rectCorner.topLeft, 0)];
    if (self.rectCorner.topLeft > 0) {
        [rounded addQuadCurveToPoint:CGPointMake(0, self.rectCorner.topLeft) controlPoint:CGPointZero];
    }
    [rounded addLineToPoint:CGPointMake(0, bounds.size.height - self.rectCorner.bottomLeft)];
    if (self.rectCorner.bottomLeft > 0) {
        [rounded addQuadCurveToPoint:CGPointMake(self.rectCorner.bottomLeft, bounds.size.height) controlPoint:CGPointMake(0, bounds.size.height)];
    }
    [rounded addLineToPoint:CGPointMake(bounds.size.width - self.rectCorner.bottomRight, bounds.size.height)];
    if (self.rectCorner.bottomRight > 0) {
        [rounded addQuadCurveToPoint:CGPointMake(bounds.size.width, bounds.size.height - self.rectCorner.bottomRight) controlPoint:CGPointMake(bounds.size.width, bounds.size.height)];
    }
    [rounded addLineToPoint:CGPointMake(bounds.size.width, self.rectCorner.topRight)];
    if (self.rectCorner.topRight > 0) {
        [rounded addQuadCurveToPoint:CGPointMake(bounds.size.width - self.rectCorner.topRight, 0) controlPoint:CGPointMake(bounds.size.width, 0)];
    }
    [rounded closePath];
    
    CAShapeLayer* shape = [CAShapeLayer layer];
    shape.frame = self.bgMaskView.bounds;
    shape.path = rounded.CGPath;
    self.bgMaskView.layer.mask = shape;
}
...
- (void)show {
    ...
    backgroundView.rectCorner = XCAlertRectCornerZero;
    ...
    if ([self.customAlert respondsToSelector:@selector(rectCornerForMaskView)]) {
        XCAlertRectCorner rectCorner = [(id<XCAlertContentProtocol>)self.customAlert rectCornerForMaskView];
        backgroundView.rectCorner = rectCorner;
    }
