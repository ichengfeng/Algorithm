...
- (void)setupUI {
    ...
    self.titleLabel.font = [UIFont systemFontOfSize:16 * adjustRatioFont weight:UIFontWeightMedium];
    ...
    self.contentLabel.font = [UIFont systemFontOfSize:14 * adjustRatioFont];
...
- (XCNormalButton *)createButtonWithAction:(XCAlertAction *)action {
    ...
    [button setCustomAttribute:^(XCButtonAttribute * _Nonnull attribute) {
        attribute.font = [UIFont systemFontOfSize:16 * adjustRatioFont];
...
- (void)layoutSubviews {
    ...
    CGSize titleSize = [self.titleLabel.text boundingRectWithSize:CGSizeMake(width - 32 * adjustRatio, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: self.titleLabel.font} context:nil].size;
    CGSize contentSize = [self.contentLabel.text boundingRectWithSize:CGSizeMake(width - 32 * adjustRatio, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: self.contentLabel.font} context:nil].size;
    self.titleLabel.frame = CGRectMake((width - titleSize.width) / 2, 10 * adjustRatio, titleSize.width, titleSize.height);
    ...
    if (titleSize.height > 0 || contentSize.height > 0) {
        buttonsView_y += 10 * adjustRatio;
        buttonsView_y += titleSize.height;
        buttonsView_y += contentSize.height;
        if (titleSize.height > 0 && contentSize.height > 0) {
            buttonsView_y += 8 * adjustRatio;
        }
        buttonsView_y += 10 * adjustRatio;
    }
    self.buttonsView.frame = CGRectMake(0, buttonsView_y, width, self.buttonsView.subviews.count * 44 * adjustRatio);
    self.topContentView.frame = CGRectMake(0, 0, width, CGRectGetMaxY(self.buttonsView.frame));
    self.cancelButtonView.frame = CGRectMake(0, CGRectGetMaxY(self.topContentView.frame) + 10 * adjustRatio, width, 44 * adjustRatio);
    ...
    for (int i = 0; i < self.buttonsView.subviews.count; i++) {
        ...
        button.frame = CGRectMake(0, button_y, width, 44 * adjustRatio);
        button_y += 44 * adjustRatio;
    }
...
- (CGRect)frameForViewContent {
    ...
    CGFloat width = screenSize.width - 32 * adjustRatio;
    CGFloat height = 0;
    CGSize titleSize = [self.titleLabel.text boundingRectWithSize:CGSizeMake(width - 32 * adjustRatio, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: self.titleLabel.font} context:nil].size;
    CGSize contentSize = [self.contentLabel.text boundingRectWithSize:CGSizeMake(width - 32 * adjustRatio, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: self.contentLabel.font} context:nil].size;
    if (titleSize.height > 0 || contentSize.height > 0) {
        height += 10 * adjustRatio;
        height += titleSize.height;
        height += contentSize.height;
        if (titleSize.height > 0 && contentSize.height > 0) {
            height += 8 * adjustRatio;
        }
        height += 10 * adjustRatio;
    }
    height += self.buttonsView.subviews.count * 44 * adjustRatio;
    height += self.cancelButton ? 44 * adjustRatio : 0;
    if (self.cancelButton != nil && self.buttonsView.subviews.count > 0) {
        height += 10 * adjustRatio;
    }
    height += 16 * adjustRatio;
