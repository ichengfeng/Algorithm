...
- (XCNormalButton *)createButtonWithAction:(XCAlertAction *)action {
    ...
    [button setCustomAttribute:^(XCButtonAttribute * _Nonnull attribute) {
        attribute.font = [UIFont systemFontOfSize:16 * adjustRatioFont weight:UIFontWeightMedium];
...
- (void)layoutSubviews {
    ...
    self.titleLabel.frame = CGRectMake((width - titleSize.width) / 2, 20 * adjustRatio, titleSize.width, titleSize.height);
    CGFloat space = (titleSize.height > 0 && contentSize.height > 0) ? 16 * adjustRatio : 0;
    ...
    CGFloat buttonViewHeight = self.buttonViews.count > 2 ? 43 * adjustRatio * self.buttonViews.count : 43 * adjustRatio;
    ...
    if (self.buttonViews.count == 2) {
        ...
        for (int i = 0; i < self.buttonViews.count; i++) {
            ...
            button.frame = CGRectMake(0, button_y, CGRectGetWidth(self.buttonContentView.frame), 43 * adjustRatio);
            button_y += 43 * adjustRatio;
        }
    }
...
- (CGRect)frameForViewContent {
    ...
    CGFloat width = 270 * adjustRatio;
    CGFloat height = 0;
    
    CGSize titleSize = [self.titleLabel.text boundingRectWithSize:CGSizeMake(width - 32 * adjustRatio, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: self.titleLabel.font} context:nil].size;
    CGSize contentSize = [self.contentLabel.text boundingRectWithSize:CGSizeMake(width - 32 * adjustRatio, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: self.contentLabel.font} context:nil].size;
    height += 20 * adjustRatio;
    height += titleSize.height;
    height += contentSize.height;
    if (titleSize.height > 0 && contentSize.height > 0) {
        height += 16 * adjustRatio;
    }
    height += 19 * adjustRatio;
    if (self.buttonViews.count > 2) {
        height += self.buttonViews.count * 43 * adjustRatio;
    } else {
        height += 43 * adjustRatio;
    }
